	...
	return (tree: Root) => {
		visit(tree, (node) => {
			if (
				...
				// @ts-expect-error: directive node has name property
				...
				const title =
					// @ts-expect-error: directive node may have attributes
				...
				// @ts-expect-error: directive node has children property
				node.children = [
					{
						type: "paragraph",
						data: {
